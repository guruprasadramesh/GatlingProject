...
class LoadBasicSimulationViaProxyAddress extends Simulation{
  ...
  val testScenario: ScenarioBuilder = scenario("Basic Load Simulation").exec(getAUser())

  setUp(testScenario.inject(nothingFor(5),atOnceUsers(5), rampUsers(15) during(5 seconds))).protocols(httpConfiguration)
}
